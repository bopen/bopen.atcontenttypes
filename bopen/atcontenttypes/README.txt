...
In this section we are tesing the RichPage content type by performing
basic operations like adding, updadating and deleting RichPage content
items.

Adding a new RichPage content item
--------------------------------
...
'RichPage' and click the 'Add' button to get to the add form.

    >>> browser.getControl('RichPage').click()
    >>> browser.getControl(name='form.button.Add').click()
    >>> 'RichPage' in browser.contents
    True

Now we fill the form and submit it.

    >>> browser.getControl(name='title').value = 'RichPage Sample'
    >>> browser.getControl('Save').click()
    >>> 'Changes saved' in browser.contents
    True

And we are done! We added a new 'RichPage' content item to the portal.

Updating an existing RichPage content item
---------------------------------------

Let's click on the 'edit' tab and update the object attribute values.

    >>> browser.getLink('Edit').click()
    >>> browser.getControl(name='title').value = 'New RichPage Sample'
    >>> browser.getControl('Save').click()
...
We check that the changes were applied.
    ...
    >>> 'Changes saved' in browser.contents
    True
    >>> 'New RichPage Sample' in browser.contents
    True

Removing a/an RichPage content item
--------------------------------

If we go to the home page, we can see a tab with the 'New RichPage
...
Now we are going to delete the 'New RichPage Sample' object. First we
go to the contents tab and select the 'New RichPage Sample' for
deletion.

    >>> browser.getLink('Contents').click()
    >>> browser.getControl('New RichPage Sample').click()
...
So, if we go back to the home page, there is no longer a 'New RichPage
Sample' tab.
...
Adding a new RichPage content item as contributor
------------------------------------------------

Not only site managers are allowed to add RichPage content items, but
...
We select 'RichPage' and click the 'Add' button to get to the add form.
...
Done! We added a new RichPage content item logged in as contributor.
